...
`fantasy-lineup-coach`-Skill führt sie aus) in
[tools/fmlib.py](../fantasy-lineup-coach/tools/fmlib.py) → `injury_gate(status, practice)`:
...
Deterministisch umgesetzt (nur der `fantasy-lineup-coach`-Skill führt sie
aus) in [tools/fmlib.py](../fantasy-lineup-coach/tools/fmlib.py) → `evaluate(...)`.
Basisgewichte:
